...
#include "McpBlueprintGraphEditorBridge.generated.h"
...
class UMcpBlueprintGraphPin;

/**
 * Object oriented helper for editing graphs in a blueprint object
 * Instantiate the UMcpBlueprintGraphEditorBridge via CreateAndEditFunctionGraph
 * or GetGraphEditor. Static helpers can be found in BlueprintEditorLibrary
 * and pin specific routines are encapsulated by McpBlueprintGraphPin
 *
 * Python Example:

import unreal

McpBlueprintGraphEditorBridge = unreal.McpBlueprintGraphEditorBridge

print_string = "/Script/Engine.KismetSystemLibrary.PrintString"

def GenHelloWorld(bp):
    editor = McpBlueprintGraphEditorBridge.create_and_edit_function_graph(bp, "HelloWorld")
...
class BLUEPRINTEDITORBRIDGE_API UMcpBlueprintGraphEditorBridge : public UObject
{
	GENERATED_BODY()
private:
	...
	UMcpBlueprintGraphEditorBridge();

public:
	/** returns a new UMcpBlueprintGraphEditorBridge for a graph in InBlueprint, creating a new graph named FuncName - useful for creating new function graphs */
	UFUNCTION(BlueprintCallable, Category = "Blueprint Editor")
	static UMcpBlueprintGraphEditorBridge* CreateAndEditFunctionGraph(UBlueprint* Blueprint, const FString& FuncName = FString(TEXT("NewFunction")));

	/** returns a new UMcpBlueprintGraphEditorBridge for a graph in InBlueprint named GraphName - useful for editing graphs that already exist */
	UFUNCTION(BlueprintCallable, Category = "Blueprint Editor")
	static UMcpBlueprintGraphEditorBridge* GetGraphEditorByName(UBlueprint* InBlueprint, FName GraphName);

	/** returns a new UMcpBlueprintGraphEditorBridge for the provided graph - useful for editing graphs that already exist */
	UFUNCTION(BlueprintCallable, Category = "Blueprint Editor")
	static UMcpBlueprintGraphEditorBridge* GetGraphEditor(UEdGraph* InGraph);
	...
	void ListAvailableNodes(TArray<FString>& Result, const TArray<FMcpBlueprintGraphPin>& ContextPins) const;
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Blueprint Editor", meta = (AutoCreateRefTerm = "ContextPins"))
	UEdGraphNode* CreateNodeFromName(const FString& NodeWithCategory, FVector2D const Location, const TArray<FMcpBlueprintGraphPin>& ContextPins, UClass* DeclaringClass = nullptr);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Blueprint Editor")
	FMcpBlueprintGraphPin AddGraphInputParameter(FName InputName, const FEdGraphPinType& PinType = FEdGraphPinType(), const FString& Value = FString(TEXT("")));
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Blueprint Editor")
	FMcpBlueprintGraphPin FindGraphEntryPin() const;
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Blueprint Editor")
	bool AddLocalVariable(FName LocalName, const FEdGraphPinType& PinType = FEdGraphPinType(), const FString& Value = FString(TEXT("")));
	/**
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Blueprint Editor")
	bool AddMemberVariable(FName MemberName,  const FEdGraphPinType& PinType = FEdGraphPinType(), const FString& Value = FString(TEXT("")));

	/**
	 * Adds a Blueprint event dispatcher and its signature graph.
	 */
	UFUNCTION(BlueprintCallable, Category = "Blueprint Editor")
	UEdGraph* AddEventDispatcher(FName DispatcherName);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Blueprint Editor")
	UK2Node_VariableGet* AddGetMemberVariableNode(FName MemberName, const FString& ClassPath = FString(TEXT("")));
	/**
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Blueprint Editor")
	UK2Node_VariableSet* AddSetMemberVariableNode(FName MemberName, const FString& ClassPath = FString(TEXT("")));
	/**
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Blueprint Editor")
	bool RemoveNodePin(UEdGraphNode* Node, const FMcpBlueprintGraphPin& Pin);
